...
class Dynamics:
    def __init__(self, constants: Dict[sympy.Symbol, float], variables: List[sympy.Symbol], dynamics):
        ...
        self._differentialEquations = {var: sympy.diff(self._dynamics, var) for var in self._variables}
    ...
    def calculateErrorMargins(self, predictedInputs: Dict[sympy.Symbol, float], errors: Dict[sympy.Symbol, float],
        ...
        listToFloat = lambda lst: [float(x) for x in lst]
        X = listToFloat(self._dynamics.subs(allVariables).n())
        dF_dx = {symb: self._differentialEquations[symb].subs(allVariables).n() for symb in predictedInputs}
        print(X)
        print(dF_dx)
        print([list(dF_dx[x]) for x in dF_dx])
        dX = sum(errors.get(symb, 0) * np.array(dF_dx[symb]) for symb in predictedInputs)[0]
        print('Predicted output:', X)
        print('Differential variances:', dX)
        # outputsForDifferentErrors = []
        # relevantErrorVariables = {symb: error for symb, error in errors.items() if symb in predictedInputs}
        # relevantErrorVariables.update({symb: 0 for symb in self._constants})
        ...
        boundingBox = [(x - dx, x + dx) for x, dx in zip(X, dX)]
