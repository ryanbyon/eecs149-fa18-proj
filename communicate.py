...
def send_path_on_bluetooth(sock, path):
	for i, path_element in enumerate(path):		
		if i % 2 == 1:
			distanceToMove = round(path_element*0.715, 2)
			# sleeping = 2
		else:
			distanceToMove = round(path_element*1, 2)
			sleeping = 1.5
...
send_path_on_bluetooth(sock, first_path[:4])
...
while not done:
	...
	send_path_on_bluetooth(sock, current_path[:4])
	count += 1
	print(counter)
	counter += 1
sock.close()
